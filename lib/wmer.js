...
function WmerClient(config) {
    ...
    if (!config.login) {
        ...
        this.setEndPoint = function(path) {
            var url = (this.baseUrl + '/ru/' + path).toString().replace(/^:\/\//g, '/');
            debug(url);
            return url;
        };
...
WmerClient.prototype.operationMobile = function(orderId, phone, amount, description) {
    ...
    return self.getProviderByPhone(phone).then(function (res) {
        if (!res.providerId) {
            throw new Error('Can not detect providerId for phone number \'' + phone + '\'');
        } else {
